...
/// For move parsing. TODO: remove if find() is tested.
...
fn gen(pos: *const Position, comptime gen_flags: u4, noalias storage: anytype) void {
    ...
    if (!doublecheck) {
        ...
        const target = if (check) pos.checkmask else if (!noisy) bb_not_us else bb_them;
        const our_pawns = pos.pawns(us);
        const our_knights = pos.knights(us);
        const our_queens_bishops = pos.queens_bishops(us);
        const our_queens_rooks = pos.queens_rooks(us);
    ...
    if (popcnt(to_bb) > 2) {
        //const bb_unsafe: u64 = get_unsafe_squares_for_king(pos, us); // pos.attacks_by_for_occupation(us.opp(), pos.all() ^ pos.kings(us));

        const bb_without_king = pos.all() ^ pos.kings(us);
        ...
        if (!check and !noisy) {
            inline for (Castle.all) |ct| {
                //if (pos.is_castling_allowed(us, ct) and is_castlingpath_empty(pos, us, ct) and is_legal_castle(pos, us, ct, bb_unsafe)) {
                if (is_castling_ok(pos, us, ct, bb_unsafe)) {
        ...
        if (!check and !noisy) {
            inline for (Castle.all) |ct| {
                //if (pos.is_castling_allowed(us, ct) and is_castlingpath_empty(pos, us, ct) and is_legal_castle_check_attacks(pos, us, ct)) {
                if (is_castling_ok_check_attacks(pos, us, ct)) {
...
fn find(pos: *const Position, comptime gen_flags: u4, from_sq: Square, to_sq: Square, prom: PieceType) !ExtMove {
    ...
    const pins_orth: u64 = if (has_pins) pos.pins_orth[us.u] else 0;
    const pins: u64 = if (has_pins) pins_diag | pins_orth else 0;
    const target = if (check) pos.checkmask else bb_not_us;
    var bb: u64 = undefined;
    ...
    sw: switch (pt.e) {
        .pawn => {
            ...
            const our_pawn = pos.pawns(us) & bb_from_filter & pawn_filter;
            ...
            if (!is_promotion) {
                ...
                const prom_flag = prom.to_promotion_move_flag();
                if (dst.single != 0) {
        ...
        .knight => {
            const our_knight = pos.knights(us) & bb_from_filter;
        ...
        .bishop, .rook, .queen => {
            const our_queen_or_bishop = pos.queens_bishops(us) & bb_from_filter;
            ...
            const our_queen_or_rook = pos.queens_rooks(us) & bb_from_filter;
        ...
        .king => {
            ...
            if (!check) {
                inline for (Castle.all) |ct| {
                    if (to_sq.u == pos.layout.rook_start(us, ct).u and is_castling_ok(pos, us, ct, bb_unsafe)) {
...
inline fn is_castling_ok(pos: *const Position, comptime us: Color, comptime ct: Castle, bb_unsafe: u64) bool {
    return
        pos.is_castling_allowed(us, ct) and
        pos.layout.empty_path(us, ct) & pos.all() == 0 and
        // is_castlingpath_empty(pos, us, ct) and
        is_legal_castle(pos, us, ct, bb_unsafe);
}

inline fn is_castling_ok_check_attacks(pos: *const Position, comptime us: Color, comptime ct: Castle) bool {
    return
        pos.is_castling_allowed(us, ct) and
        pos.layout.empty_path(us, ct) & pos.all() == 0 and
        //is_castlingpath_empty(pos, us, ct) and
        is_legal_castle_check_attacks(pos, us, ct);
}

/// Checks the castling attack path against unsafe squares.
fn is_legal_castle(pos: *const Position, comptime us: Color, comptime ct: Castle, bb_unsafe: u64) bool {
    // Frc requires this additional pin check. In classic the rooks cannot be pinned.
    if (pos.pins_orth[us.u] & pos.layout.rook_start(us, ct).to_bitboard() != 0) return false;
    //const path: u64 = pos.layout.attack_path(us, ct);
    return  pos.layout.attack_path(us, ct) & bb_unsafe == 0;
}

/// Checks each castling attack path square.
fn is_legal_castle_check_attacks(pos: *const Position, comptime us: Color, comptime ct: Castle) bool {
    const them: Color = comptime us.opp();
    // Frc requires this additional pin check. In classic the rooks cannot be pinned.
    if (pos.pins_orth[us.u] & pos.layout.rook_start(us, ct).to_bitboard() != 0) return false;
    var path: u64 = pos.layout.attack_path(us, ct);
