...
fn gen(pos: *const Position, comptime gen_flags: u4, noalias storage: anytype) void {
    ...
    if (!doublecheck) {
        ...
        if (our_pawns != 0) {
            ...
            const seventh_rank: u64 = comptime bitboards.relative_rank_bitboard(us, types.rank_7);
            const last_rank: u64 = comptime bitboards.relative_rank_bitboard(us, types.rank_8);
...
inline fn compute_pawn_destinations(comptime us: Color, pawns: u64, empty_squares: u64, enemies: u64, check_interpolation: u64, pins: u64, pins_orth: u64, pins_diag: u64) PawnDestinations {
    const third_rank: u64 = comptime bitboards.relative_rank_bitboard(us, types.rank_3);
